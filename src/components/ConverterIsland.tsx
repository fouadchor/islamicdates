...
export default function ConverterIsland({ lang }: Props) {
  ...
  if (mode === 'g2h') {
    ...
    } else { convResult = pick(lang, 'اختر تاريخاً', 'Pick a date', 'تاریخ منتخب کریں'); }
  } else {
  ...
  return (
    <section className="card" style={{ padding:'22px 26px' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12, marginBottom:18 }}>
        <h2 style={{ margin:0, fontWeight:700, fontSize:18 }}>{pick(lang, 'محوّل التاريخ السريع', 'Quick Date Converter', 'فوری تاریخ کنورٹر')}</h2>
        <div style={{ display:'flex', gap:3, background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:999, padding:4 }}>
          <button style={pillS(mode==='g2h')} onClick={() => setMode('g2h')}>{pick(lang, 'ميلادي ← هجري', 'Gregorian → Hijri', 'عیسوی ← ہجری')}</button>
          <button style={pillS(mode==='h2g')} onClick={() => setMode('h2g')}>{pick(lang, 'هجري ← ميلادي', 'Hijri → Gregorian', 'ہجری ← عیسوی')}</button>
        </div>
      </div>

      {mode === 'g2h' ? (
        <label style={{ display:'flex', flexDirection:'column', gap:7, fontSize:13, color:'var(--muted)', maxWidth:260 }}>
          {pick(lang, 'اختر تاريخاً ميلادياً', 'Pick a Gregorian date', 'عیسوی تاریخ منتخب کریں')}
      ...
      <div style={{ marginTop:20, padding:'18px 20px', borderRadius:14, background:'var(--accent-soft)' }}>
        <div style={{ fontSize:12, color:'var(--accent)', fontWeight:700, letterSpacing:'.06em', textTransform:'uppercase', marginBottom:7 }}>
          {pick(lang, 'النتيجة', 'Result', 'نتیجہ')}
        </div>
