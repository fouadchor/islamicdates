...
const CITIES: City[] = [
  ...
  { key: 'jakarta',   ar: 'جاكرتا',     en: 'Jakarta',       lat: -6.2088, lng: 106.8456, zone: 'Asia/Jakarta',      method: 'MWL' },
];

// Offset in hours (incl. DST) for an IANA zone at a given date.
...
interface Loc { lat: number; lng: number; zone: string; label: string }
...
export default function PrayerTimesIsland({ lang }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      else { const c = CITIES[0]; setLoc({ lat: c.lat, lng: c.lng, zone: c.zone, label: ar ? c.ar : c.en }); setMethod(c.method); }
    } catch {
      const c = CITIES[0]; setLoc({ lat: c.lat, lng: c.lng, zone: c.zone, label: ar ? c.ar : c.en });
    }
  ...
  const useMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        setLoc({ lat: pos.coords.latitude, lng: pos.coords.longitude, zone, label: ar ? 'موقعي الحالي' : 'My location' });
        setGeo('idle');
      },
      () => setGeo('denied'),
  ...
  const pickCity = (key: string) => {
    ...
    setLoc({ lat: c.lat, lng: c.lng, zone: c.zone, label: ar ? c.ar : c.en });
    setMethod(c.method);
  };
  ...
  // current decimal hour in the location's zone
  ...
  // current prayer = last passed among fajr..isha
    ...
    <section style={{ ...card, padding: '24px 26px', animation: 'fadeUp .5s ease' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
        <div>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, marginTop: 4 }}>{loc.label}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: ar ? 'flex-start' : 'flex-end' }}>
          ...
          <select value="" onChange={e => pickCity(e.target.value)}
