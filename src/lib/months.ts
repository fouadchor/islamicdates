...
export function monthBasePath(hy: number, hm: number, ar: boolean): string {
  return ar ? `/${hy}/${monthSlug(hm)}/` : `/en/${hy}/${monthSlug(hm)}/`;
}
...
function occHref(hm: number, hd: number, date: Date, ar: boolean): string | null {
  ...
  for (const y of [gy - 1, gy, gy + 1]) {
    ...
    if (occurrencesInGYear(def.hm, def.hd, y).some(o => Math.abs(o.date.getTime() - date.getTime()) < 2 * 86400000)) {
      return occBasePath(def.slug, y, ar);
    }
  }
  return null;
}
...
export function monthGrid(hy: number, hm: number, ar: boolean): MDay[] {
  ...
  for (let guard = 0; guard < 32; guard++) {
    ...
    if (raw) {
      const [cat, arName, enName] = raw;
      occ = { cat, name: ar ? arName : enName, href: occHref(hm, h.d, date, ar) };
    }
